...
Flash_Storage = st.selectbox('Flash_Storage(in GB)', [0, 8, 128, 256, 512, 1024])
...
if st.button('Predict Price'):
    ...
    input_df = pd.DataFrame({'Company': [company], 'TypeName': [type], 'Ram': [ram], 'OpSys': [os], 'Weight': [
                            weight], 'TouchScreen': [touchscreen], 'IPS': [ips], 'PPI': [ppi], 'CPU_name': [cpu], 'HDD': [hdd], 'SSD': [ssd], 'Flash_Storage': [Flash_Storage], 'GPU_Brand': [gpu]})

    #st.write(input_df)

    prediction = int(np.exp(rf.predict(input_df)))
